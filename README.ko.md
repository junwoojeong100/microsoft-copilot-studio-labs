...
두 언어 가이드는 **한국어 자막·실습 프롬프트·응답을 사용하는 동일한 영상**으로 안내합니다. 영상은 가상 데이터를 사용하는 편집된 실습 예시입니다.
...
실행 코드·식별자·한국어 입력 및 상태 값은 표시된 그대로 사용하세요. 영문 설명은 의미를 안내하는 용도이며 실제 입력값을 번역문으로 바꾸지 않습니다. `<LAB_PREFIX>` 등의 자리표시자는 각 가이드의 사전 준비에 있는 이름 규칙에 따라 바꿉니다.
